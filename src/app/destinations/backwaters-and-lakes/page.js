...
const backwaterDestinations = [
  {
    id: "01",
    name: "Kumarakom",
    tag: "Vembanad Lake",
    desc: "Vembanad Lake's tranquil shores — the most romantic backwater wedding destination in India. Experience intimate ceremonies surrounded by serene waters and lush tropical beauty.",
    img: "couple-shots/TSR53127.jpg",
    pills: ["Lakeside Venues", "Tranquil Shores", "Romantic Backwaters"]
  },
  {
    id: "02",
    name: "Kochi",
    tag: "Colonial Charm",
    desc: "Where history meets modernity — colonial forts, heritage palaces, and waterfront luxury. Kochi offers a truly unique cultural blend for an unforgettable celebration.",
    img: "couple-shots/0G4A4625.jpg",
    pills: ["Heritage Palaces", "Colonial Forts", "Waterfront Luxury"]
  }
];

export default function BackwatersAndLakesPage() {
    ...
    <div ref={containerRef} className="bg-bg overflow-x-hidden flex flex-col gap-0 p-0 m-0">
      ...
      <section className="relative h-screen w-full flex items-center justify-center overflow-hidden m-0 p-0">
        <div className="absolute inset-0 z-0">
          <Image
            ...
            priority
            sizes="100vw"
            {...getBlurProps("/assets/photos/destination/backwaterandlakes.jpg")}
        ...
        <div className="relative z-10 text-center px-6 reveal">
          ...
          <p className="text-gold text-[12px] tracking-[0.6em] uppercase mb-4 font-medium max-w-[600px] mx-auto leading-[1.8]">Houseboats, lake palaces, and still waterways — India's most unique and intimate wedding settings.</p>
          <h1 className="font-heading text-surface text-6xl md:text-8xl font-light leading-tight mb-4 mt-6">
            Backwaters <br />
            <em className="italic">& Lakes</em>
          </h1>
          <GoldDivider darkBg flip className="mt-8 mx-auto" />
        </div>
      </section>
      ...
      <section className="p-0 m-0 border-none">
        <div className="flex flex-col gap-0 p-0 m-0">
          {backwaterDestinations.map((dest, i) => {
            const isEven = i % 2 === 0;
            return (
              <div 
                key={dest.id} 
      ...
      <section className="bg-[#1a1200] py-20 px-12 text-center relative overflow-hidden m-0">
        ...
        <div className="relative z-10 reveal">
          <p className="text-gold text-[12px] tracking-[0.6em] uppercase mb-6 font-medium">Your Tranquil Escape</p>
